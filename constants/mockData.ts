...
export type FinancePeriodData = {
  ...
  // rootCauseKpiKey · rootCauseReason 제거 (근본):
  //   이전엔 mock에 하드코딩된 kpiKey·reason이 pickRootCause 계산 결과와 어긋나
  //   헤더의 이름·값·이유가 서로 다른 KPI를 가리키는 3원 어긋남 버그가 있었음.
  //   이제 dashboard가 pickRootCause 결과에서 generateRootCauseReason으로 동적 생성.
