...
export const FINANCE_DATA_BY_PERIOD: Record<"today" | "week" | "month" | "quarter", FinancePeriodData> = {
  today: {
    ...
    overallVerdict: "오늘 하루 수익성(돈 버는 힘) 성숙 · 유지(환자 다시 오게 하는 힘) 정상 · 리스크(돈 새는 곳) 관리 진행 중",
    rootCauseKpiKey: "recallRate",
    rootCauseReason: "리콜 성공률 63%(정기 검진 안내 후 실제 예약 성공률)가 재내원율·환자 LTV(환자 한 명이 평생 낼 돈)·예방매출·순이익률 4개의 상류(원인 지표). 여기부터 손대면 4개 지표 동시 회복.",

    profitabilityInsight:
      "일 매출 310만 · 순이익률 21.4%(매출에서 모든 비용 빼고 남는 비율) · LTV:CAC 3.5x(환자 1명 평생 매출이 데려오는 비용의 3.5배 · 3~5배 정상) · 인당 순이익 83만원 = 오늘 하루는 벌었지만 안정기 병원 인당 목표(200만원/월)의 41% 수준 — 오늘 지표 유지가 관건.",
    retentionInsight:
      "재내원율 76%(치료 끝난 환자가 다시 오는 비율 · 최우수) 대비 리콜 성공률 63%(정기 검진 안내→실제 예약 비율 · 경고)로 gap 13%p. 예방 매출 비중 7.1%(전체 매출 중 스케일링/검진 비중)는 위기 임계값(12%) 아래로 절반 수준.",
    riskInsight:
      "노쇼 5.0%(예약해놓고 안 온 환자) + 당일 취소 6.7% + 상담 거절 50% + 미수금 99만원(아직 못 받은 진료비). 오늘 매출 누수 추정 42만원 — 리마인드 문자 2건과 미회수 콜 1건으로 즉시 방어 가능.",
    overallPrescription: {
      analysis: [
  ...
  week: {
    ...
    overallVerdict: "이번 주 수익성(돈 버는 힘) 성숙 · 유지(환자 재방문) 균열 시작 · 리스크(돈 새는 곳) 확대 중",
    rootCauseKpiKey: "recallRate",
    rootCauseReason: "리콜 성공률 62%(정기 검진 안내 후 실제 예약 성공률 · 경고)가 재내원율·환자 LTV(환자 1명 평생 매출)·예방매출·순이익률 4개의 상류(원인 지표). 이번 주 리콜 도래 환자 38명에 예약 문자 발송 시 3~4주 뒤 4개 지표 동시 회복.",

    profitabilityInsight:
      "주 매출 1,860만 · 순이익률 19.2%(매출에서 모든 비용 뺀 뒤 남는 비율) · LTV:CAC 3.6x(환자 1명 평생 매출 ÷ 데려오는 비용) = Unit Economics(환자 1인당 수익성 분석) 정상 구간 유지 중. 인건비 대비 순이익 배수 0.53x(인건비 1원당 순이익 53원 창출) — 안정기 목표(1.0x)의 절반이라 확장 투자는 아직 이름.",
    retentionInsight:
      "재내원율 75%(치료 끝난 환자 재방문 비율) 대비 리콜 성공률 62% gap 13%p로 재내원 파이프에서 새는 중. 예방 매출 6.5%(전체 매출 중 스케일링/검진 비중)는 위기 임계값(12%) 아래 — NRR 축(환자 유지 경제학) 균열 신호.",
    riskInsight:
      "노쇼 7.4%(예약해놓고 안 온 환자 비율) + 당일 취소 7.5% + 상담 거절 51% + 미수금 624만원(아직 못 받은 진료비). 이번 주 매출 누수 추정 145만원 — 리마인드 자동화·분납 옵션·회수 콜로 이 중 90만원 방어 가능.",
    overallPrescription: {
      analysis: [
  ...
  month: {
    ...
    overallVerdict: "이번 달 수익성(돈 버는 힘) 성숙 · 유지(환자 재방문) 균열 · 리스크(돈 새는 곳) 확대 — 3축 불균형",
    rootCauseKpiKey: "recallRate",
    rootCauseReason: "리콜 성공률 61%(정기 검진 안내 후 실제 예약 성공률 · 경고)가 재내원율·환자 LTV(환자 1명 평생 매출)·예방매출·순이익률·마케팅 ROI(광고비 1원당 회수 매출) 5개의 상류(원인 지표). 스케일링 리콜 캠페인 4주 집행 시 5개 지표 동시 회복 예상.",

    profitabilityInsight:
      "월 매출 7,850만 · 순이익률 17.0%(매출에서 모든 비용 뺀 뒤 남는 비율 · 안정기 목표 22% 대비 -5%p) · LTV:CAC 3.7x(환자 1명 평생 매출 ÷ 데려오는 비용) · BEP 10일(월초 10일에 손익분기 넘음). Unit Economics(환자 1인당 수익성) 정상이나 순이익률 하락 추세 — 리콜/재내원 회복 없으면 다음 분기 15%대까지 후퇴 우려.",
    retentionInsight:
      "재내원율 74%(치료 끝난 환자 재방문 비율 · 최우수) 지지대 위에 리콜 성공률 61%·예방 매출 6.2%(스케일링/검진 매출 비중 · 위기) 두 축 흔들림. NRR(신환보다 기존 환자 유지가 더 이익이라는 방법론) 관점에서는 기존 환자의 재구매 파이프가 4주 안에 멈출 리스크.",
    riskInsight:
      "노쇼 8.7%(예약해놓고 안 온 환자 비율) + 당일 취소 8.7% + 상담 거절 60% + 미수금 3,689만원(아직 못 받은 진료비). 이번 달 매출 누수 720만원 확정 — 리마인드 자동화 + 분납 옵션 + 회수 콜 세 액션으로 480만원 즉시 방어.",
    overallPrescription: {
      analysis: [
  ...
  quarter: {
    ...
    overallVerdict: "이번 분기 수익성(돈 버는 힘) 성숙 · 유지(환자 재방문) 위기 진입 · 리스크(돈 새는 곳) 확대 — 구조 조정 필요",
    rootCauseKpiKey: "recallRate",
    rootCauseReason: "리콜 성공률 58%(정기 검진 안내 후 실제 예약 성공률 · 위기 진입)가 유지 축 3개 지표(재내원율·환자 LTV·예방매출)를 동시에 끌어내리는 중. 분기 리콜 파이프라인 재설계 없이는 다음 분기 LTV(환자 1명 평생 매출)·순이익률 추가 하락 확실.",

    profitabilityInsight:
      "분기 매출 2억 1,850만 · 순이익률 16.3%(매출에서 모든 비용 뺀 뒤 남는 비율 · 3개월 연속 하락) · LTV:CAC 3.9x(환자 1명 평생 매출 ÷ 데려오는 비용). 인당 순이익 4,453만원은 안정기 벤치마크 대비 상위지만 감소 추세 — 유지 축 개선이 다음 분기 수익성 유지의 유일한 지렛대.",
    retentionInsight:
      "재내원율 72%(치료 끝난 환자 재방문 비율)·리콜 성공률 58%(위기)·예방 매출 5.9%(스케일링/검진 매출 비중 · 위기) — NRR(환자 유지 경제학) 3지표 중 2개가 위기. 신환 유치보다 5배 저비용인 유지 파이프가 무너지는 중이라 분기 재설계 시급.",
    riskInsight:
      "노쇼 6.7%(예약해놓고 안 온 환자 비율 · 개선) + 당일 취소 7.8% + 상담 거절 62% + 미수금 1억 264만원(아직 못 받은 진료비). 90일 초과 미수금 884만원은 회수 확률 14%로 사실상 손실 확정 — 이번 분기 안에 회수 아니면 대손(못 받는 돈으로 처리) 검토.",
    overallPrescription: {
      analysis: [
