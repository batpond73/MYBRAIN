...
export const FINANCE_DATA_BY_PERIOD: Record<"today" | "week" | "month" | "quarter", FinancePeriodData> = {
  today: {
    ...
    rootCauseReason:
      "리콜 성공률 63%가 재내원율·환자 LTV·예방매출·순이익률 4개의 상류. 여기부터 손대면 4개 지표 동시 회복.\n(정기 검진 안내를 받은 환자 10명 중 6~7명만 예약을 잡고 있어요. 이 숫자 하나가 원장님 병원 4가지 문제(다시 오는 환자 비율·환자 1명 평생 매출·정기 진료 매출·순이익)의 뿌리입니다. 여기만 개선해도 4가지가 함께 좋아져요.)",

    profitabilityInsight:
      "일 매출 310만 · 순이익률 21.4% · LTV:CAC 3.5x · 인당 순이익 83만원 = 오늘 하루는 벌었지만 안정기 병원 인당 목표(200만원/월)의 41% 수준 — 오늘 지표 유지가 관건.\n(오늘 매출 310만원에서 66만원(21.4%)이 순이익으로 남았어요. 환자 한 명 유치 비용에 비해 그 환자가 병원에 낼 돈은 3.5배로 정상 구간이고요. 다만 스태프 1인당 오늘 만든 순이익 83만원은 안정된 병원의 월 목표(200만원)에 비하면 41%밖에 안 됩니다. 오늘 이 페이스가 이번 달 내내 유지되어야 목표에 다가갈 수 있어요.)",
    retentionInsight:
    ...
    riskInsight:
      "노쇼 5.0% + 당일 취소 6.7% + 상담 거절 50% + 미수금 99만원. 오늘 매출 누수 추정 42만원 — 리마인드 문자 2건과 미회수 콜 1건으로 즉시 방어 가능.\n(오늘 예약해놓고 안 온 환자 5%, 당일 취소한 환자 6.7%, 상담 후 치료 안 하겠다고 한 환자가 절반, 아직 못 받은 진료비 99만원이 있어요. 이걸 다 합치면 오늘 하루에만 약 42만원이 그냥 새어나갔습니다. 예약 전날 문자 2건과 밀린 진료비 전화 1건만으로 대부분 방어 가능해요.)",
    overallPrescription: {
      analysis: [
  ...
  week: {
    ...
    rootCauseKpiKey: "recallRate",
    rootCauseReason:
      "리콜 성공률 62%(경고)가 재내원율·환자 LTV·예방매출·순이익률 4개의 상류. 이번 주 리콜 도래 환자 38명에 예약 문자 발송 시 3~4주 뒤 4개 지표 동시 회복.\n(이번 주에 정기 검진 예약 안내를 받은 환자 38명이 있는데 이 중 62%만 예약을 잡고 있어요. 이 사람들 전원에게 예약 문자를 다시 보내면, 3~4주 뒤부터 다시 오는 환자·환자 평생 매출·정기 진료 매출·순이익 4가지가 함께 좋아집니다.)",
    ...
    riskInsight:
      "노쇼 7.4% + 당일 취소 7.5% + 상담 거절 51% + 미수금 624만원. 이번 주 매출 누수 추정 145만원 — 리마인드 자동화·분납 옵션·회수 콜로 이 중 90만원 방어 가능.\n(이번 주에 예약 안 나온 환자 7.4%, 당일 취소 7.5%, 상담 후 치료 안 하겠다고 한 환자가 절반, 아직 못 받은 진료비 624만원이 있어요. 이걸 합치면 이번 주 매출에서 약 145만원이 그냥 새어나갔습니다. 예약 자동 문자·치료비 나눠 내기 안내·미수금 회수 전화 세 가지만 시작해도 90만원은 바로 지킬 수 있어요.)",
    overallPrescription: {
      analysis: [
  ...
  month: {
    ...
    rootCauseKpiKey: "recallRate",
    rootCauseReason:
      "리콜 성공률 61%(경고)가 재내원율·환자 LTV·예방매출·순이익률·마케팅 ROI 5개의 상류. 스케일링 리콜 캠페인 4주 집행 시 5개 지표 동시 회복 예상.\n(정기 검진 안내를 받은 환자 10명 중 6명만 예약을 잡고 있는데, 이 숫자 하나가 원장님 병원 5가지 문제(다시 오는 환자·환자 1명 평생 매출·정기 진료 매출·순이익·광고 효율)의 뿌리예요. 스케일링·검진 안내 문자 캠페인을 4주만 돌려도 이 5가지가 한꺼번에 좋아집니다.)",

    profitabilityInsight:
      "월 매출 7,850만 · 순이익률 17.0%(연차 대비 -5%p) · LTV:CAC 3.7x · BEP 10일. Unit Economics 정상이나 순이익률 하락 추세 — 리콜/재내원 회복 없으면 다음 분기 15%대까지 후퇴 우려.\n(이번 달 매출 7,850만원 중 순이익이 1,335만원(17.0%) 남았어요. 안정된 병원 목표는 22%인데 5%p 낮은 상태이고, 3개월 연속 떨어지는 중입니다. 다행히 환자 유치 비용 대비 그 환자가 낼 돈은 3.7배로 좋은 구조이고, 월초 10일 만에 그달 모든 비용을 커버해서 나머지 20일은 순수 이익 구간이에요. 하지만 재방문·리콜 회복 없으면 다음 분기 순이익률이 15%대까지 밀릴 수 있습니다.)",
    retentionInsight:
    ...
    riskInsight:
      "노쇼 8.7% + 당일 취소 8.7% + 상담 거절 60% + 미수금 3,689만원. 이번 달 매출 누수 720만원 확정 — 리마인드 자동화 + 분납 옵션 + 회수 콜 세 액션으로 480만원 즉시 방어.\n(이번 달 예약해놓고 안 온 환자 8.7%, 당일 취소도 8.7%, 상담 후 치료 안 하겠다고 한 환자가 60%, 아직 못 받은 진료비가 3,689만원 있어요. 이걸 다 합치면 이번 달에만 720만원이 그냥 새어나갔습니다. 예약 전날 자동 문자·치료비 나눠 내기 안내·미수금 회수 전화 세 가지만 시작해도 이 중 480만원은 바로 지킬 수 있어요.)",
    overallPrescription: {
      analysis: [
  ...
  quarter: {
    ...
    rootCauseKpiKey: "recallRate",
    rootCauseReason:
      "리콜 성공률 58%(위기 진입)가 유지 축 3개 지표를 동시에 끌어내리는 중. 분기 리콜 파이프라인 재설계 없이는 다음 분기 LTV·순이익률 추가 하락 확실.\n(정기 검진 안내를 받은 환자 10명 중 5.8명만 예약을 잡고 있는데, 이 숫자 하나가 다시 오는 환자·환자 평생 매출·정기 진료 매출 3가지를 동시에 끌어내리고 있어요. 분기 단위로 정기 검진 안내 방식(문자 시점·문구·담당자 지정)을 다시 짜지 않으면 다음 분기 순이익률도 추가 하락 확정입니다.)",

    profitabilityInsight:
      "분기 매출 2억 1,850만 · 순이익률 16.3%(3개월 연속 하락) · LTV:CAC 3.9x. 인당 순이익 4,453만원은 안정기 벤치마크 대비 상위지만 감소 추세 — 유지 축 개선이 다음 분기 수익성 유지의 유일한 지렛대.\n(분기 매출 2억 1,850만원 중 순이익이 16.3%로 남았는데, 3개월 연속 떨어지고 있어요. 환자 유치 비용 대비 그 환자가 낼 돈은 3.9배로 여전히 좋은 구조이고, 스태프 1인당 분기 순이익 4,453만원도 다른 병원보다 앞선 편이지만 계속 줄어드는 추세입니다. 즉 지금 구조는 좋지만 뒷심이 약해지고 있어서, 다시 오는 환자·정기 검진 파이프를 회복하는 것 외에는 순이익률을 되돌릴 방법이 없어요.)",
    retentionInsight:
    ...
    riskInsight:
      "노쇼 6.7%(개선) + 당일 취소 7.8% + 상담 거절 62% + 미수금 1억 264만원. 90일 초과 미수금 884만원은 회수 확률 14%로 사실상 손실 확정 — 이번 분기 안에 회수 아니면 대손 처리 검토.\n(이번 분기 예약해놓고 안 온 환자 비율은 6.7%로 조금 좋아졌지만, 당일 취소·상담 거절은 여전하고 아직 못 받은 진료비가 무려 1억 264만원 쌓였어요. 그중 90일 넘게 방치된 884만원은 회수 확률이 14%라 사실상 못 받는 돈이 됐습니다. 분기 마감 전에 못 받으면 회계상 '못 받는 돈'으로 정리(대손)해서 재무제표를 깨끗이 하는 걸 검토해야 해요.)",
    overallPrescription: {
      analysis: [
